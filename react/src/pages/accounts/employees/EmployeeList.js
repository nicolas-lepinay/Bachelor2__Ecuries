...
function EmployeeList() {
    ...
    const handleUpdate = async (newData) => {
        try {
            ...
            const newState = employees.map(user => {
    ...
    return (
        <PageWrapper title={adminMenu.accounts.accounts.subMenu.professionals.text}>
            <Page >
                <div className='h1 font-family-playfair d-flex justify-content-center mb-5'>Liste des professionnels</div>
                <div className='row row-cols-lg-2 row-cols-1 mt-5'>
                    {employees.map((user) => (
                        <div key={user.username} className='col mx-auto my-1'>
                            <Card >
                                <CardBody>
